...
function showAuthRequired() {
    checkoutContainer.innerHTML = `
        <div class="auth-required">
            ...
            <a href="login.html?redirect=checkout.html" class="btn-shop-now">
                <i class="fas fa-sign-in-alt"></i> Login to Continue
            </a>
            <p style="margin-top: 15px; font-size: 13px; color: #aaa;">
                Don't have an account? <a href="register.html" style="color: #FF9900;">Register</a>
            </p>
        </div>
    `;
}
...
async function loadCheckoutSummary() {
    ...
    try {
        ...
        apiCart = await apiFetch(`${API_BASE_URL}/api/cart/`);
        console.log('Cart response:', apiCart);
    } catch (error) {
        console.warn('API cart failed, using localStorage:', error.message);
        useLocalCart = true;
    }

    // If API returned null (401 handled) or empty, use localStorage
    ...
    const itemsHTML = cart.map((item, index) => `
        <div class="checkout-item">
            <div class="item-image">
                ${item.image_url 
                    ? `<img src="${item.image_url}" alt="${item.name}" onerror="this.style.display='none'; this.parentElement.innerHTML='<i class=\'fas fa-image placeholder-icon\'></i>';">`
        ...
        <div class="checkout-grid">
            <div class="checkout-items">
                <div class="checkout-header">
                    <h2><i class="fas fa-shopping-bag"></i> Order Items (${itemCount})</h2>
                </div>
                <div class="items-list">
                    ${itemsHTML}
                </div>
                <a href="cart.html" class="btn-edit-cart">
            ...
            <div class="checkout-summary">
                ...
                <div class="shipping-form">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="ship-name" value="${user?.name || ''}" placeholder="Enter your full name">
                    </div>
                    <div class="form-group">
                        <label>Phone Number</label>
                        <input type="tel" id="ship-phone" value="${user?.phone || ''}" placeholder="e.g. 0712345678">
                    </div>
                    <div class="form-group">
                        <label>Delivery Address</label>
                        <textarea id="ship-address" rows="3" placeholder="Enter your delivery address"></textarea>
                    </div>
                    <div class="form-group">
                        <label>City / Town</label>
                        <input type="text" id="ship-city" placeholder="e.g. Nairobi, Mombasa">
                    </div>
                </div>
                    ...
                    <div class="payment-options">
                        <label class="payment-option">
                            ...
                            <div class="payment-label">
                                <i class="fas fa-mobile-alt" style="color: #27ae60;"></i>
                                <div>
                        ...
                        <label class="payment-option">
                            ...
                            <div class="payment-label">
                                <i class="fas fa-hand-holding-usd" style="color: #FF9900;"></i>
                                <div>
                ...
                <button class="btn-place-order" onclick="placeOrder()">
                    <i class="fas fa-lock"></i> Place Order — KES ${Number(total).toLocaleString()}
                </button>
                <p style="text-align: center; font-size: 12px; color: #888; margin-top: 10px;">
                    <i class="fas fa-shield-alt"></i> Secure checkout powered by Kenya Marketplace
                </p>
            </div>
        </div>
    `;
    ...
    try {
        const orderData = {
            ...
            total_amount: cart.reduce((sum, item) => sum + (item.price * item.quantity), 0) + 
                         (cart.reduce((sum, item) => sum + (item.price * item.quantity), 0) > 5000 ? 0 : 300)
        };
        ...
        if (!result) return; // Auth error handled
