...
def ensure_cart_table():
    ...
    try:
        ...
        print(f"Cart table creation error (may already exist): {e}")
...
def add_to_cart(item: dict, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Add to cart error: {str(e)}")
...
def update_cart_item(cart_item_id: int, item: dict, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Update cart error: {str(e)}")
...
def remove_from_cart(cart_item_id: int, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Remove from cart error: {str(e)}")
