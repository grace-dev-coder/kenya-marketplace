...
async def create_admin(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        phone = body.get("phone", "+254700000000")  # Default phone for admin
        ...
        admin = User(
            ...
            password_hash=hashed_password,
            full_name=full_name,
            phone=phone,  # Now always provided
            is_admin=True,
            is_vendor=False
        )
        db.add(admin)
        db.commit()
        return {"message": "Admin created successfully", "email": email, "phone": phone}
